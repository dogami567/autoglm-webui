...
def check_system_requirements(
    ...
    Checks:
    - ADB/HDC/iOS tools installed
    - At least one device connected
    - ADB Keyboard installed on the device (for ADB only)
    - ADB input injection available (for ADB only)
    - WebDriverAgent running (for iOS only)
    ...
    if device_type == DeviceType.ADB:
        ...
        if all_passed:
            print("4. Checking ADB input injection...", end=" ")
            try:
                result = subprocess.run(
                ...
                if result.returncode == 0:
                    ...
                    print("❌ FAILED")
                    print("   Error: ADB input (tap) failed.")
                    if output:
                        print(f"   Detail: {output}")
                    print("   Solution:")
                    print("     1. Re-authorize USB debugging on the device")
                    print("     2. Some OEM ROMs require enabling extra settings in Developer options:")
                    print("        - USB debugging (Security settings) / Allow simulated input")
                    print("        - Install via USB (if applicable)")
                    all_passed = False
            except subprocess.TimeoutExpired:
                print("❌ FAILED")
                print("   Error: ADB command timed out.")
