...
- do(action="Paste", text="xxx")  
    Paste是“剪贴板粘贴”输入：先把文本写入系统剪贴板，再触发粘贴到当前输入框。不依赖 ADB Keyboard，适合某些 App 不允许切换/启用 ADB Keyboard 的场景。若一次粘贴失败，可尝试先长按输入框并点击“粘贴”。
- do(action="Paste_Stream", text="xxx", chunks=["这个","真的","好好吃","！"])  
    Paste_Stream是“分段粘贴”输入：把文本拆成多个短片段逐段粘贴（chunks 可省略，系统会按标点/长度粗略拆分）。适合某些 App 对一次性长文本输入/粘贴不稳定的场景。
- do(action="Interact")  
