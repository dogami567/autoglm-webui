...
def _run_adb(
    ...
    *,
    ...
    """
    Run an ADB command and raise on failure.

    NOTE: Some OEM ROMs block input injection unless the user enables extra
    developer options (e.g. "USB debugging (Security settings)"). If that
    happens, `adb shell input ...` can fail and we want to surface stderr.
    """
    ...
    if cp.returncode != 0:
        ...
        raise RuntimeError(f"ADB command failed ({cp.returncode}): {' '.join(cmd)}\n{detail}")
    return cp
...
def tap(
    ...
    _run_adb(adb_prefix, ["shell", "input", "tap", str(x), str(y)])
    time.sleep(delay)
...
def back(device_id: str | None = None, delay: float | None = None) -> None:
    ...
    _run_adb(adb_prefix, ["shell", "input", "keyevent", "4"])
    time.sleep(delay)
...
def home(device_id: str | None = None, delay: float | None = None) -> None:
    ...
    _run_adb(adb_prefix, ["shell", "input", "keyevent", "KEYCODE_HOME"])
    time.sleep(delay)
