...
def set_clipboard(text: str, device_id: str | None = None) -> None:
    ...
    This is useful as a fallback when ADB Keyboard can't be enabled in some apps.
    It relies on Android's built-in `cmd clipboard` service (available on most modern ROMs).
    """
    adb_prefix = _get_adb_prefix(device_id)
    result = subprocess.run(
        adb_prefix + ["shell", "cmd", "clipboard", "set", text],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        output = (result.stdout + result.stderr).strip()
        raise RuntimeError(output or "Failed to set clipboard (cmd clipboard set)")


def paste_clipboard(device_id: str | None = None) -> None:
    """
    Paste clipboard into the currently focused field.

    NOTE: This sends a keyevent (KEYCODE_PASTE=279). Some apps/ROMs may block it.
    """
    adb_prefix = _get_adb_prefix(device_id)
    result = subprocess.run(
    ...
    if result.returncode != 0:
        output = (result.stdout + result.stderr).strip()
        raise RuntimeError(output or "Failed to paste clipboard (input keyevent 279)")
