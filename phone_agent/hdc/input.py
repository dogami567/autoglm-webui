...
def set_clipboard(text: str, device_id: str | None = None) -> None:
    """
    Set device clipboard text (HDC).

    NOTE: Not implemented yet. HarmonyOS clipboard APIs differ by version/ROM.
    """
    raise NotImplementedError("HDC clipboard set is not implemented")


def paste_clipboard(device_id: str | None = None) -> None:
    """
    Paste clipboard into the currently focused field (HDC).

    NOTE: Not implemented yet. Use `type_text` or UI interactions for now.
    """
    raise NotImplementedError("HDC clipboard paste is not implemented")
