...
class DeviceFactory:
    ...
    def set_clipboard(self, text: str, device_id: str | None = None):
        """Set clipboard text (if supported by the device backend)."""
        if hasattr(self.module, "set_clipboard"):
            return self.module.set_clipboard(text, device_id)
        raise NotImplementedError(f"set_clipboard is not supported for {self.device_type.value}")

    def paste_clipboard(self, device_id: str | None = None):
        """Paste clipboard into focused field (if supported by the device backend)."""
        if hasattr(self.module, "paste_clipboard"):
            return self.module.paste_clipboard(device_id)
        raise NotImplementedError(f"paste_clipboard is not supported for {self.device_type.value}")
